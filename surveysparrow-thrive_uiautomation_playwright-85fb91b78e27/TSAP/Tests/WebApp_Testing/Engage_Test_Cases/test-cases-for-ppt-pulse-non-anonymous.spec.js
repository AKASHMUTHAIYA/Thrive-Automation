...
test.describe("PPT Export - Pulse Non-Anonymous Survey", () => {
	let poManager;
	let commonFunctions;
	let pptExportPage;
	let surveyPage;
	...
	test("TC_01_to_TC_05_PPT Pulse Non-Anonymous: slides UI, global config, default and filtered data @pulse @non-anonymous @ppt @regression  ", async ({
		thrivePage,
		browser,
	}) => {
		await allure.step(
			"Navigate to Engage tab and create non-anonymous pulse survey",
			async () => {
				await commonFunctions.navigateTopNavigateSection("Engage");
				await surveyPage.createNewSurvey(
					`Pulse Non-Anonymous PPT Survey ${time}`,
				);
			...
			await allure.step(
				...
				async () => {
					...
					if (hasFilter) {
						...
						expect(
							chipTexts.length,
							"Pulse surveys always originate Rounds filters",
						).toBe(2);
						expect(
							chipTexts,
							"PPT should show the default Rounds filter chips",
						).toEqual(expect.arrayContaining(["Rounds:", "All Rounds"]));
					}
					await pptExportPage.closePPTConfiguration();
