...
test.describe("Add a survey collaborator and verify their functionality", () => {
	test("Add employee manually and send invite in one flow @Regression @surveyCollaborator", async ({
		...
		await allure.description(
			"Creates one employee manually, sends invite immediately, and verifies Active status (and invite email) and adds a collaborator to the survey",
		);
		...
		await allure.step("Create survey and add collaborator", async () => {
			await commonFunctions.navigateTopNavigateSection("Engage");

			const surveyName = `engage survey for colloborator${time}`;
